  ...
  const renderGlobalPeriodTable = () => {
    ...
    const htmlRows = pageRows.length ? pageRows.map((row, index) => {
      ...
      <tr class="${trace.internalUse ? 'inventario-internal-use-row' : 'inventario-trace-row'}">
        ...
        <td>${trace.internalUse ? '<div class="inventario-internal-meta"><span class="inventario-resolution-badge">Uso interno en empresa</span><small>Sin trazabilidad</small></div>' : `<button type="button" class="btn ios-btn ios-btn-secondary inventario-threshold-btn" data-open-production-trace="${escapeHtml(trace.productionId)}"><i class="fa-solid fa-users-viewfinder"></i><span>trazabilidad</span></button>`}</td>
      </tr>`).join('') : '';
      ...
      return `<tr class="inventario-row-tone ${isExpiredAvailable ? 'is-expired-row' : ''} ${resolutionLabel ? 'is-resolution-row' : ''} ${index % 2 === 0 ? 'is-even-row' : 'is-odd-row'}">
        ...
        <td>${escapeHtml(row.noPerecedero ? 'No perecedero' : (row.expiryDate || '-'))} </td>
  ...
  const renderEntryTable = (record) => {
    ...
    const rowsHtml = pageRows.length ? pageRows.map((entry, index) => {
      ...
      <tr class="inventario-row-tone ${isExpiredAvailable ? 'is-expired-row' : ''} ${resolutionLabel ? 'is-resolution-row' : ''} ${index % 2 === 0 ? 'is-even-row' : 'is-odd-row'}">
        ...
        <td>${entry.noPerecedero ? 'No perecedero' : (entry.expiryDate || '-')} </td>
