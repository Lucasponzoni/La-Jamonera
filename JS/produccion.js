(function produccionModule() {
  ...
  const renderDispatchCreate = (draft) => {
    ...
    const commentRows = draft.comments.map((comment, idx) => `<tr class="dispatch-comment-row"><td colspan="5"><input class="form-control ios-input" data-dispatch-comment="${idx}" placeholder="Comentario visual (no afecta stock)" value="${escapeHtml(comment)}"></td><td><button type="button" class="btn family-manage-btn" data-dispatch-comment-remove="${idx}"><i class="fa-solid fa-trash"></i></button></td></tr>`).join('');
