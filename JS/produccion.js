(function produccionModule() {
  ...
  const nodes = {
    ...
    recipeGroups: document.getElementById('produccionRecipeGroups'),
  ...
  const RECIPE_GROUPS_PATH = '/recetas_groups';
  ...
  const pendingRegistroDetailIds = new Set();
  const pendingInventoryDetailIds = new Set();
  const failedRegistroDetailIds = new Set();
  const failedInventoryDetailIds = new Set();
  ...
  const appendRecipeMovement = (recipeId, movement = {}) => {
    ...
    const qtyKg = type === 'egreso' ? Math.abs(toFiniteKg(movement.qtyKg)) : toFiniteKg(movement.qtyKg);
  ...
  const isIsoDateToken = (value) => /^\d{4}-\d{2}-\d{2}$/.test(normalizeValue(value));
  const getEntryDateIso = (entry) => {
    const entryDate = normalizeValue(entry?.entryDate);
    return isIsoDateToken(entryDate) ? entryDate : '';
  };
  const isEntryFutureForProductionDate = (entry, productionDateIso) => {
    const entryDate = getEntryDateIso(entry);
    const productionDate = normalizeValue(productionDateIso);
    return Boolean(entryDate && productionDate && entryDate > productionDate);
  };
  const minIsoDate = (dates = []) => dates.filter(isIsoDateToken).sort()[0] || '';
  const maxIsoDate = (dates = []) => dates.filter(isIsoDateToken).sort().slice(-1)[0] || '';
  ...
  const getProductExpiryStatusMeta = (registro) => {
    const expiryIso = resolveProductExpiryIso(registro);
    if (!expiryIso) return { expiryIso: '', label: 'Sin VTO', tone: 'is-warning', expired: false };
  ...
  const formatKgBadgeValue = (value) => {
    ...
    return amount.toLocaleString('es-AR', { minimumFractionDigits: 0, maximumFractionDigits: 3 });
  };
  const renderProductExpiryCell = (registro) => {
  ...
  const isRegistroDetailHydrated = (registro = {}) =>
    Boolean(registro?.__detailLoaded) || Array.isArray(registro?.lots);
  const ensureRegistroDetail = async (key) => {
    const id = normalizeValue(key);
    if (!id) return null;
    const current = safeObject(state.registros?.[id]);
    if (current.id && !current.__indexLite && isRegistroDetailHydrated(current)) return current;
    try {
      const detail = safeObject(await window.dbLaJamoneraRest.read(`${REGISTROS_PATH}/${id}`));
      if (detail.id || Object.keys(detail).length) {
        const merged = { ...current, ...detail, id, __indexLite: false, __detailLoaded: true };
        merged.traceCount = getTraceRowsFromRegistro(merged).length;
        state.registros[id] = merged;
        failedRegistroDetailIds.delete(id);
        return state.registros[id];
      }
    } catch (error) {
    }
    failedRegistroDetailIds.add(id);
    return current.id ? current : null;
  };
  const hydrateRegistroDetailsForRows = (rows = [], onDone = null, limit = 12) => {
    const ids = [...new Set((Array.isArray(rows) ? rows : [])
      .map((row) => normalizeValue(row?.id))
      .filter((id) => id && !failedRegistroDetailIds.has(id) && !isRegistroDetailHydrated(state.registros?.[id]) && !pendingRegistroDetailIds.has(id)))]
      .slice(0, limit);
    if (!ids.length) return false;
    ids.forEach((id) => pendingRegistroDetailIds.add(id));
    Promise.all(ids.map((id) => ensureRegistroDetail(id)))
      .finally(() => {
        ids.forEach((id) => pendingRegistroDetailIds.delete(id));
    ...
    await Promise.all(uniqueIds.map(async (ingredientId) => {
      ...
      try {
        ...
        if (Object.keys(detail).length) {
          ...
          failedInventoryDetailIds.delete(ingredientId);
        } else {
          failedInventoryDetailIds.add(ingredientId);
        }
      } catch (error) {
        failedInventoryDetailIds.add(ingredientId);
      }
  ...
  const hydrateInventoryDetailsForRecipes = (recipes = [], onDone = null, limit = 30) => {
    const ids = [...new Set((Array.isArray(recipes) ? recipes : []).flatMap((recipe) => getRecipeIngredientIds(recipe))
      .map(normalizeValue)
      .filter((id) => {
        if (!id || pendingInventoryDetailIds.has(id) || failedInventoryDetailIds.has(id)) return false;
        const current = safeObject(state.inventario.items?.[id]);
        return !Array.isArray(current.entries) || current.__indexLite;
      }))].slice(0, limit);
    if (!ids.length) return false;
    ids.forEach((id) => pendingInventoryDetailIds.add(id));
    ensureInventoryDetailsForIds(ids)
      .then(() => {
        (Array.isArray(recipes) ? recipes : []).forEach((recipe) => {
          if (recipe?.id) state.analysis[recipe.id] = analyzeRecipe(recipe);
        });
      })
      .finally(() => {
        ids.forEach((id) => pendingInventoryDetailIds.delete(id));
        if (typeof onDone === 'function') onDone();
      });
    return true;
  };
  ...
  const getInventoryAvailability = (ingredientId, targetUnit, productionDateIso = toIsoDate()) => {
    ...
    const aggregate = entries.reduce((acc, entry) => {
      ...
      if (entryMeta.category === targetMeta.category) {
        ...
        if (futureForDate) {
          acc.futureBase += netBase;
          acc.hasFuture = true;
          if (entryDate && (!acc.nextEntryDate || entryDate < acc.nextEntryDate)) acc.nextEntryDate = entryDate;
          return acc;
        }
      ...
      return acc;
    }, { totalBase: 0, usableBase: 0, futureBase: 0, incompatible: [], hasExpired: false, hasFuture: false, nextToExpire: null, nextEntryDate: '' });
    return {
      available: fromBase(aggregate.usableBase, targetUnit),
      total: fromBase(aggregate.totalBase, targetUnit),
      futureAvailable: fromBase(aggregate.futureBase, targetUnit),
      hasExpired: aggregate.hasExpired,
      hasFuture: aggregate.hasFuture,
      incompatibleUnits: aggregate.incompatible,
      nextToExpire: aggregate.nextToExpire,
      nextEntryDate: aggregate.nextEntryDate
    };
  };
  ...
  const getRequirementRelatedOptions = (row, neededPerKg, unit, productionDateIso) => normalizeRelatedIngredients(row?.relatedIngredients)
    .map((item) => {
      ...
      const futureCoverageKg = neededPerKg > 0 ? (availability.futureAvailable / neededPerKg) : 0;
      return {
        ...
        futureAvailable: availability.futureAvailable,
        coverageKg: Math.max(0, maxCoverageKg),
        totalCoverageKg: Math.max(0, totalCoverageKg),
        futureCoverageKg: Math.max(0, futureCoverageKg),
        hasExpired: availability.hasExpired,
        hasFuture: availability.hasFuture,
        nextEntryDate: normalizeValue(availability.nextEntryDate),
  ...
  const allocateAcrossFefoGroup = ({ entries = [], remaining = 0, requirement, productionDateIso, warnings, loteAntiguo = false }) => {
    ...
    const availableGroup = entries.map((entry) => {
      ...
      const expiryIso = isEntryNoPerecedero(entry) ? '' : normalizeValue(entry.expiryDate);
      const futureForDate = isEntryFutureForProductionDate(entry, productionDateIso);
      const status = futureForDate ? 'future' : (!expiryIso || expiryIso >= productionDateIso ? 'ok' : 'expired');
      const isSoon = status === 'ok' && expiryIso && expiryIso >= productionDateIso && expiryIso <= toIsoDate(new Date(productionDateIso).getTime() + 2 * 86400000);
    ...
    const eligible = availableGroup.filter((item) => item.status === 'ok');
    ...
    availableGroup.forEach((item) => {
      ...
      let take = 0;
      if (item.status === 'ok' && totalEligible > 0 && pending > 0) {
      ...
      lots.push({
        ...
        takeQty: item.status !== 'ok' ? 0 : Number(take.toFixed(4)),
        takeBaseQty: item.status !== 'ok' ? 0 : Number(toBase(take, requirement.unit).toFixed(6)),
        ...
        status: item.status === 'future' ? 'future' : (item.status === 'expired' ? 'expired' : (item.isSoon ? 'soon' : 'ok'))
      });
    });
    ...
    if (extraRemaining > 0.0001) {
      lots.forEach((lot) => {
        if (extraRemaining <= 0.0001 || lot.status !== 'ok') return;
  ...
  const analyzeRecipe = (recipe, productionDateIso = toIsoDate()) => {
    ...
    rows.forEach((row) => {
      ...
      const futureCoverage = neededPerKg > 0 ? Math.max(0, availability.futureAvailable) / neededPerKg : 0;
      ...
      const effectiveFutureCoverage = computeMaxKgWithCaps([{ coverageKg: coverage + futureCoverage, maxShare: 1 }, ...relatedOptions.map((item) => ({ coverageKg: Number(item.coverageKg || 0) + Number(item.futureCoverageKg || 0), maxShare: item.maxShare }))]);
      ...
      const nextEntryDate = minIsoDate([
        normalizeValue(availability.nextEntryDate),
        ...relatedOptions.map((item) => normalizeValue(item.nextEntryDate))
      ]);
      ...
      requirements.push({
        ...
        directFutureCoverage: futureCoverage,
        coverage: effectiveCoverage,
        totalCoverage: effectiveTotalCoverage,
        futureCoverage: Math.max(0, effectiveFutureCoverage - effectiveCoverage),
        relatedOptions,
        ...
        hasFuture: availability.hasFuture || relatedOptions.some((item) => item.hasFuture),
        nextEntryDate,
    ...
    const futureRequirements = requirements.filter((item) => item.hasFuture && item.missingForMin > 0.0001);
    const nextAvailableDate = maxIsoDate(futureRequirements.map((item) => item.nextEntryDate));
    const futureKg = futureRequirements.reduce((sum, item) => sum + Number(item.futureAvailable || 0), 0);
    ...
    return {
      ...
      hasExpired,
      hasFuture: requirements.some((item) => item.hasFuture),
      futureRequirements,
      nextAvailableDate,
      futureKg: Number(futureKg.toFixed(4)),
      hasInfiniteStock,
      minKg,
      expiredKg
    };
  ...
  const buildPlanForRecipe = (recipe, qtyKg, productionDateIso = toIsoDate(), options = {}) => {
    ...
    analysis.requirements.forEach((requirement) => {
      ...
      const consumeIngredientSource = ({ ingredientId, ingredientName, maxShare = 1, isSubstitute = false }) => {
        ...
        const availableQty = lots
          .filter((lot) => lot.status !== 'expired' && lot.status !== 'future')
          .reduce((sum, lot) => sum + Number(lot.availableQty || 0), 0);
        ingredientPlans.push({
          ingredientId,
          ingredientName,
          ingredientUnit: requirement.unit,
      ...
      const allocatePooled = (sources, poolNeed) => {
        ...
        substituteGroups.forEach((group) => {
          ...
          let groupRemaining = Math.min(remaining, prepared.reduce((sum, item) => sum + (item.status !== 'ok' ? 0 : Math.min(item.availableInReqUnit, perRelatedCap[item.related.ingredientId] || 0)), 0));
          for (let pass = 0; pass < 6 && groupRemaining > 0.0001; pass += 1) {
            const eligible = prepared.filter((item) => item.status === 'ok' && item.availableInReqUnit - item.takeQty > 0.0001 && (perRelatedCap[item.related.ingredientId] || 0) > 0.0001);
          ...
          prepared.forEach((item) => {
            ...
            const lot = {
              ...
              invoiceImageUrls: Array.isArray(item.entry.invoiceImageUrls) ? item.entry.invoiceImageUrls : (item.entry.invoiceImageUrl ? [item.entry.invoiceImageUrl] : []),
              unit: requirement.unit,
              takeQty: item.status !== 'ok' ? 0 : Number(item.takeQty.toFixed(4)),
              takeBaseQty: item.status !== 'ok' ? 0 : Number(toBase(item.takeQty, requirement.unit).toFixed(6)),
              availableQty: Number(item.available.toFixed(4)),
              availableKg: getEntryAvailableKg(item.entry),
              entryAvailableQty: Number(item.available.toFixed(4)),
              status: item.status === 'future' ? 'future' : (item.status === 'expired' ? 'expired' : (item.isSoon ? 'soon' : 'ok'))
      ...
      if (missing > 0.0001) {
        if (options.loteAntiguo) {
          ...
          if (existingPlan) {
            ...
            existingPlan.noTraceability = true;
            existingPlan.sinTrazabilidad = true;
          } else {
            ingredientPlans.push({
              ingredientId: requirement.ingredientId,
              ingredientName: requirement.name,
              ingredientUnit: requirement.unit,
              neededQty: Number(missing.toFixed(4)),
              availableQty: 0,
              missingQty: Number(missing.toFixed(4)),
              sourceIngredientId: requirement.ingredientId,
              sourceIngredientName: requirement.name,
              substitutionLabel: '',
              isSubstitute: false,
              noTraceability: true,
              sinTrazabilidad: true,
              lots: []
            });
          }
        } else {
          ...
          const futureLots = lots.filter((lot) => lot.status === 'future' && Number(lot.availableQty || 0) > 0.0001);
          const nextFutureDate = minIsoDate(futureLots.map((lot) => lot.entryDate));
          const futureConflict = nextFutureDate
            ? `${requirement.name}: faltan ${formatQty(missing, requirement.unit)} para la fecha ${productionDateIso}. Hay stock con ingreso posterior (${formatIsoEs(nextFutureDate)}); cambiá la fecha de producción a esa fecha o producí sin trazabilidad para el faltante.`
            : '';
          if (hasExpiredWithStock) {
            ...
            conflicts.push(futureConflict || `${requirement.name}: faltan ${formatQty(missing, requirement.unit)} para la fecha ${productionDateIso}. Ingresá un nuevo lote o cambiá fecha.`);
          }
        }
      }
    });
    ...
    }))).filter((lock) => Number(lock.reservedQty || 0) > 0.0001);
    return {
      recipeId: recipe.id,
  ...
  const hasRegistroTracePreview = (registro = {}) => {
    if (getTraceRowsFromRegistro(registro).length > 0) return true;
    if (failedRegistroDetailIds.has(normalizeValue(registro?.id))) return false;
    return !isRegistroDetailHydrated(registro) && Number(registro?.traceCount || 0) > 0;
  };
  const isRegistroDetailLoading = (registro = {}) => pendingRegistroDetailIds.has(normalizeValue(registro?.id));
  // Celda "Adjuntos" del historial: con el índice lite los lots todavía no están,
  // así que mostramos "Cargando" hasta que el detalle hidrate (en vez de un falso
  // "Sin adjuntos").
  ...
  const markProductionExport = async (productionId, type) => {
    ...
    await window.dbLaJamoneraRest.write(`${REGISTROS_PATH}/${productionId}`, reg);
  ...
  const getTraceSafeNodeId = (value, fallback) => {
  ...
  const buildTraceMermaidDefinition = (registro) => {
    ...
    const esc = (value) => String(value || '-')
      ...
      .replace(/>/g, '&gt;')
      .replace(/"/g, '&quot;');
    const groupedIngredients = getTraceIngredientGroups(registro);
    ...
    groupedIngredients.forEach((group, index) => {
      ...
      const frozenQtyByUnit = frozenLots.reduce((acc, lot) => {
        ...
        acc[key] = (acc[key] || 0) + Number(lot?.takeQty || 0);
        return acc;
      }, {});
      const frozenSummary = frozenLots.length
        ? `<b>${frozenLots.length} ${frozenLots.length === 1 ? 'lote congelado' : 'lotes congelados'}${Object.keys(frozenQtyByUnit).length ? ` (${esc(Object.entries(frozenQtyByUnit).map(([unit, qty]) => formatCompactQty(qty, unit === 'unidad' ? '' : unit)).join(' + '))})` : ''} a -18 grados</b>`
        : '';
      const nodeId = getTraceSafeNodeId(`ING_${index + 1}_${group?.sourceIngredientId || ''}`, `ING_${index + 1}`);
      const nodeLabel = [
  ...
  const renderHistoryTable = () => {
    ...
    // Hidratamos todas las filas visibles (no sólo las expandidas): la columna
    // "Adjuntos" necesita los lots del detalle, que el índice lite no trae.
    hydrateRegistroDetailsForRows(pageRows, renderHistoryTable, PAGE);
    ...
    const htmlRows = pageRows.length ? pageRows.map((item, index) => {
      ...
      const hasTracePreview = hasRegistroTracePreview(item);
  ...
  const syncRecipeGroupsVisibility = () => {
    const shouldHide = Boolean(state.historyMode || state.dispatchMode);
    nodes.data?.classList.toggle('is-history-mode', Boolean(state.historyMode));
    nodes.data?.classList.toggle('is-dispatch-mode', Boolean(state.dispatchMode));
    if (!nodes.recipeGroups) return;
    nodes.recipeGroups.classList.toggle('d-none', shouldHide);
    nodes.recipeGroups.hidden = shouldHide;
    nodes.recipeGroups.style.display = shouldHide ? 'none' : '';
    if (shouldHide) {
      nodes.recipeGroups.setAttribute('aria-hidden', 'true');
    } else {
      nodes.recipeGroups.removeAttribute('aria-hidden');
    }
  };
  ...
  const getDispatchAllocationDisplay = (item = {}, allocation = {}) => {
    ...
    const amount = Math.abs(Number(allocation.qty || 0)) > 0
      ? Math.abs(Number(allocation.qty || 0))
      : (['unidad', 'unidades', 'u', 'un'].includes(normalizedUnit)
        ? Math.abs(Number(item.qty || 0))
        : Math.abs(Number(allocation.qtyKg || item.qtyKg || 0)));
    return {
      amount,
      unit,
  ...
  const getDispatchLocationLabel = (dispatchRow = {}, clientInput = {}) => {
    const client = { ...safeObject(clientInput) };
  ...
  const getDispatchGroupedProducts = (dispatchRow = {}) => {
    ...
    products.forEach((item, index) => {
      ...
      const normalizedItem = { ...item, allocations };
      ...
      if (dispatchRow.importedFromXlsx && parentKey) {
        ...
        groups.push({
          key: parentKey,
          label: normalizeValue(item.sourceParentLabel || item.recipeTitle || `Producto ${index + 1}`),
  ...
  const buildDispatchPlanillaHtml = (dispatchRow) => {
    ...
    const location = getDispatchLocationLabel(dispatchRow, client);
    const { groups, standalone } = getDispatchGroupedProducts(dispatchRow);
  ...
  const getDispatchAvailableByProductionId = (productionId) => {
    ...
    const directEgressKg = Object.values(safeObject(state.reparto?.productIndex?.[prod.recipeId]?.movements))
      ...
      .reduce((acc, movement) => acc + Math.abs(Number(movement?.qtyKg || 0)), 0);
    return Number(Math.max(0, producedKg - dispatchedKg - directEgressKg).toFixed(3));
  };
  const getDispatchLotProductionDate = (reg = {}) => {
    const direct = normalizeDispatchDateToken(reg.productionDate || reg.elaborationDate || reg.fechaElaboracion);
    if (direct) return direct;
    const created = Number(reg.createdAt || 0);
    return created > 0 ? toIsoDate(created) : '';
  };
  const buildRecipeLotsForDispatch = (recipeId) => getRegistrosList()
    ...
    .map((reg) => ({
      productionId: reg.id,
      lotNumber: reg.id,
      productionDate: getDispatchLotProductionDate(reg),
  ...
  const getProductionStockLotsForRecipe = (recipeId) => getRegistrosList()
    .filter((reg) => normalizeValue(reg.recipeId) === normalizeValue(recipeId) && normalizeValue(reg.status) !== 'anulada')
    .map((reg) => {
    ...
    .filter((row) => row.availableKg > 0.0001)
    .sort((a, b) => {
      const expiryA = normalizeValue(a.expiryIso) || '9999-12-31';
      const expiryB = normalizeValue(b.expiryIso) || '9999-12-31';
      if (expiryA !== expiryB) return expiryA.localeCompare(expiryB);
      return normalizeValue(a.productionDate).localeCompare(normalizeValue(b.productionDate));
    });
  const openProductionStockLotsAlert = async (recipeId) => {
    const recipe = safeObject(state.recetas?.[recipeId]);
  ...
  const isDispatchLotFutureForDate = (lot = {}, dispatchDateIso = toIsoDate()) => {
    const productionIso = normalizeValue(lot.productionDate);
    return Boolean(productionIso && dispatchDateIso && productionIso > dispatchDateIso);
  };
  ...
  const isDispatchLotUsableForDate = (lot = {}, dispatchDateIso = toIsoDate()) =>
    !isDispatchLotFutureForDate(lot, dispatchDateIso) && !isDispatchLotExpiredForDate(lot, dispatchDateIso);
  const getDispatchLotWindowLabel = (lot = {}) => {
    const from = normalizeValue(lot.productionDate);
    const to = normalizeValue(lot.expiryDate);
    if (from && to) return `del ${formatIsoEs(from)} al ${formatIsoEs(to)}`;
    if (from) return `desde ${formatIsoEs(from)}`;
    if (to) return `hasta ${formatIsoEs(to)}`;
  ...
  const allocateDispatchLots = (recipeId, qtyKg, dispatchDateIso = toIsoDate()) => {
    const needed = Number(qtyKg || 0);
    const allLots = buildRecipeLotsForDispatch(recipeId);
    const futureLots = allLots.filter((lot) => isDispatchLotFutureForDate(lot, dispatchDateIso));
    const expiredLots = allLots.filter((lot) => isDispatchLotExpiredForDate(lot, dispatchDateIso));
    const usableLots = allLots.filter((lot) => isDispatchLotUsableForDate(lot, dispatchDateIso));
    const totalStockKg = allLots.reduce((acc, lot) => acc + Number(lot.availableKg || 0), 0);
    const availableKg = usableLots.reduce((acc, lot) => acc + Number(lot.availableKg || 0), 0);
    const futureKg = futureLots.reduce((acc, lot) => acc + Number(lot.availableKg || 0), 0);
    const expiredKg = expiredLots.reduce((acc, lot) => acc + Number(lot.availableKg || 0), 0);
    const nextAvailableDate = futureLots
      .map((lot) => normalizeValue(lot.productionDate))
      .filter(Boolean)
      .sort()[0] || '';
    if (!Number.isFinite(needed) || needed <= 0) {
      return {
        ...
        expiredLots,
        fulfilledKg: 0,
        missingKg: Math.max(0, Number(needed || 0)),
        hasStock: false,
        availableKg: Number(availableKg.toFixed(3)),
        totalStockKg: Number(totalStockKg.toFixed(3)),
        futureKg: Number(futureKg.toFixed(3)),
        expiredKg: Number(expiredKg.toFixed(3)),
        nextAvailableDate
      };
    }
    let remaining = needed;
    const allocations = [];
    usableLots.forEach((lot) => {
      if (remaining <= 0.0001) return;
  ...
  const rebuildProductIndexEntryMetrics = (entry) => {
    ...
    movements.forEach((move) => {
      ...
      const qtyKg = type === 'egreso' ? Math.abs(toFiniteKg(move.qtyKg)) : toFiniteKg(move.qtyKg);
      if (type === 'egreso') {
        totalOut += qtyKg;
  ...
  const toDispatchXlsxToken = (value) => normalizeUpper(String(value || '').replace(/\s+/g, ' ').trim().replace(/[.:]/g, ''));
  const DISPATCH_XLSX_HEADERS = {
    client: ['NOMBCLI', 'NOMBREDECLIENTE', 'NOMBRECLIENTE'],
    invoice: ['NROCOMPROB', 'NROCOMPROBANTE', 'NUMERODEFACTURA', 'FACTURA'],
    date: ['FECHEMISION', 'FECHAEMISIONDEFACTURA', 'FECHAEMISION'],
    product: ['DETRENG', 'PRODUCTOS', 'PRODUCTO'],
    qty: ['CANTIDAD', 'CANTIDADENKILOS', 'KILOS'],
    observation: ['LEYENDACAB', 'OBSERVACIONES', 'OBSERVACION']
  };
  const normalizeDispatchXlsxProductKey = (value) => {
    const normalized = normalizeUpper(value).replace(/\s+/g, ' ').trim();
    return normalized.replace(/[.$#[\]/]/g, '_') || 'PRODUCTO_SIN_NOMBRE';
  };
  const getDispatchXlsxConfig = () => {
    state.reparto.xlsxConfig = safeObject(state.reparto.xlsxConfig);
  ...
  const findDispatchClientByName = (name) => {
    ...
    return Object.values(safeObject(state.reparto.clients)).find((client) => normalizeLower(client.name) === needle) || null;
  };
  const ensureDispatchXlsxClient = (name) => {
    const existing = findDispatchClientByName(name);
    if (existing?.id) return { client: existing, isNew: false };
    const cfg = getDispatchXlsxConfig();
    const next = Number(cfg.clientSequence || 0) + 1;
    cfg.clientSequence = next;
    const doc = String(next).padStart(7, '0');
    const id = makeId('dispatch_client_xlsx');
    const created = {
      id,
      name,
      doc,
      ...
      province: 'Santa Fe',
      country: 'Argentina',
      initials: initialsFromPersonName(name) || 'U',
      createdAt: nowTs(),
      source: 'xlsx_import'
    };
    state.reparto.clients[id] = created;
    return { client: created, isNew: true };
  };
  const parseDispatchXlsxQty = (value) => {
  ...
  const toDispatchXlsxPositiveQty = (value) => {
    const qty = Number(parseDispatchXlsxQty(value) || 0);
    return Number.isFinite(qty) ? Number(Math.abs(qty).toFixed(3)) : 0;
  };
  const isDispatchXlsxRowQtyValid = (row) => {
    if (!row) return false;
    const sourceQty = Number(row.sourceQty || 0);
    if (!Number.isFinite(sourceQty) || sourceQty <= 0.0001) return false;
    const mappedIngredients = Array.isArray(row.mappedIngredients) ? row.mappedIngredients : [];
    if (mappedIngredients.length) {
      return mappedIngredients.every((item) => Number(item?.qty || 0) > 0.0001);
    }
    const mappedQty = Number(row.mappedQty || 0);
    return Number.isFinite(mappedQty) && mappedQty > 0.0001;
  };
  const toDispatchXlsxIsoFromSerial = (value) => {
    const serial = Number(value);
    if (!Number.isFinite(serial)) return '';
    const utcDays = Math.floor(serial - 25569);
    const utcValue = utcDays * 86400;
    const date = new Date(utcValue * 1000);
    return Number.isNaN(date.getTime()) ? '' : toIsoDate(date.getTime());
  };
  const parseDispatchXlsxDateValue = (value) => {
    if (value instanceof Date && !Number.isNaN(value.getTime())) return toIsoDate(value.getTime());
    if (typeof value === 'number') {
      const iso = toDispatchXlsxIsoFromSerial(value);
      if (iso) return iso;
    }
    const raw = normalizeValue(value);
    if (!raw) return '';
    const normalized = normalizeDispatchDateToken(raw);
    if (normalized) return normalized;
    const loose = raw.match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})$/);
    if (loose) return `${loose[3]}-${String(loose[2]).padStart(2, '0')}-${String(loose[1]).padStart(2, '0')}`;
    return raw;
  };
  const loadSheetJsOnce = async () => {
    if (window.XLSX) return;
    await new Promise((resolve, reject) => {
      const script = document.createElement('script');
      script.src = 'https://cdn.jsdelivr.net/npm/xlsx/dist/xlsx.full.min.js';
      script.onload = resolve;
      script.onerror = () => reject(new Error('sheetjs_load_error'));
      document.head.appendChild(script);
    });
  };
  const readDispatchXlsxRows = async (file) => {
    await loadSheetJsOnce();
    const buffer = await file.arrayBuffer();
    const workbook = window.XLSX.read(buffer, { type: 'array', cellDates: true, raw: false });
    const first = workbook?.SheetNames?.[0];
    if (!first) throw new Error('sheet_missing');
    const sheet = workbook.Sheets[first];
    return window.XLSX.utils.sheet_to_json(sheet, { header: 1, defval: '' });
  };
  const findDispatchXlsxLayoutFromRows = (rows) => {
    ...
    for (let rowIdx = 0; rowIdx < maxRows; rowIdx += 1) {
      ...
      const map = {};
      row.forEach((raw, col) => {
        const token = toDispatchXlsxToken(raw);
        if (!token) return;
        Object.entries(DISPATCH_XLSX_HEADERS).forEach(([key, aliases]) => {
          if (!map[key] && aliases.includes(token)) map[key] = col;
        });
      });
      if (map.client !== undefined && map.invoice !== undefined && map.date !== undefined && map.product !== undefined && map.qty !== undefined) {
        return { headerRow: rowIdx, map };
      }
    }
    return null;
  };
  const getDispatchXlsxProductRule = (sourceName) => {
    const key = normalizeDispatchXlsxProductKey(sourceName);
  ...
  const resolveDispatchXlsxProductMeta = (sourceName, qtyRaw, options = {}) => {
    ...
    const sourceQty = toDispatchXlsxPositiveQty(qtyRaw);
    const effectiveDispatchDateIso = normalizeDispatchDateToken(options.dispatchDateIso) || toIsoDate();
    const configuredMultiplier = toDispatchXlsxPositiveQty(rule.customKg);
    if (Boolean(rule.useCustomKg) && Number(parseDispatchXlsxQty(rule.customKg) || 0) < 0) rule.customKg = configuredMultiplier;
    ...
    const ingredientBreakdown = ingredientTargets.map((targetRow) => {
      ...
      const configuredQty = toDispatchXlsxPositiveQty(targetRow.qty);
      const useSourceQty = Boolean(targetRow.useSourceQty);
      if (!useSourceQty && Number(parseDispatchXlsxQty(targetRow.qty) || 0) < 0) targetRow.qty = configuredQty;
    ...
    if (ingredientBreakdown.length) {
      return {
        rule,
        targetId: ingredientBreakdown[0].id,
        targetTitle: ingredientBreakdown.map((item) => item.title).join(' + '),
        sourceQty,
        mappedQty: ingredientBreakdown.reduce((acc, item) => acc + Number(item.qty || 0), 0),
    ...
    if (targetId) {
      if (isIngredient) {
        ...
        futureQty = Number(recipeStock.future || 0);
        nextAvailableDate = normalizeValue(recipeStock.nextAvailableDate);
        stockWindows = Array.isArray(recipeStock.windows) ? recipeStock.windows : [];
    ...
    return {
      rule,
      targetId,
      targetTitle: normalizeValue(target.title || target.name),
      sourceQty,
      mappedQty,
      availableKg,
      availableUnit,
  ...
  const applyDispatchXlsxRuleToDraftRows = (sourceProduct) => {
    if (!state.dispatchXlsxDraft || !Array.isArray(state.dispatchXlsxDraft.rows)) return;
    ...
    state.dispatchXlsxDraft.rows.forEach((row) => {
      ...
      row.mappedFutureQty = Number(meta.futureQty || 0);
      row.mappedNextAvailableDate = normalizeValue(meta.nextAvailableDate);
      row.mappedStockWindows = Array.isArray(meta.stockWindows) ? meta.stockWindows : [];
  ...
  const parseDispatchXlsxWorkbook = async (file) => {
    const rowsRaw = await readDispatchXlsxRows(file);
    const layout = findDispatchXlsxLayoutFromRows(rowsRaw);
    if (!layout) throw new Error('headers_missing');
    const rows = [];
    const clientOps = [];
    for (let rowIdx = layout.headerRow + 1; rowIdx < rowsRaw.length; rowIdx += 1) {
      const row = Array.isArray(rowsRaw[rowIdx]) ? rowsRaw[rowIdx] : [];
      const readCell = (key) => row[layout.map[key]];
      const clientName = normalizeValue(readCell('client'));
      const invoiceNumber = normalizeValue(readCell('invoice'));
      const invoiceDate = parseDispatchXlsxDateValue(readCell('date'));
      const productName = normalizeValue(readCell('product'));
      const qtyRaw = readCell('qty');
      const observation = layout.map.observation !== undefined ? normalizeValue(readCell('observation')) : '';
      ...
      rows.push({
        ...
        clientDoc: clientInfo.client.doc,
        clientStatus: clientInfo.isNew ? 'new' : 'existing',
        invoiceNumber,
        invoiceDate,
        sourceProduct: productName,
        sourceQty: meta.sourceQty,
        observation,
        mappedTargetId: meta.targetId,
        mappedTargetTitle: meta.targetTitle,
        ...
        mappedFutureQty: Number(meta.futureQty || 0),
        mappedNextAvailableDate: normalizeValue(meta.nextAvailableDate),
        mappedStockWindows: Array.isArray(meta.stockWindows) ? meta.stockWindows : [],
    ...
    return { rows, touchedClients: clientOps.some((item) => item.isNew) };
  };
  const getDispatchXlsxTargetCatalog = (targetType) => {
    if (normalizeValue(targetType) === 'ingredient') {
      return Object.values(safeObject(state.ingredientes)).map((item) => ({
        id: normalizeValue(item.id),
        title: normalizeValue(item.name || item.title),
        imageUrl: sanitizeImageUrl(item.imageUrl)
      })).filter((item) => item.id && item.title);
    }
    return Object.values(safeObject(state.recetas)).map((item) => ({
      id: normalizeValue(item.id),
  ...
  const getDispatchXlsxRecipeStockMeta = (recipeId, dispatchDateIso = toIsoDate()) => {
    ...
    const totals = lots.reduce((acc, lot) => {
      ...
      if (isDispatchLotFutureForDate(lot, dispatchDateIso)) {
        acc.future += qty;
        const date = normalizeValue(lot.productionDate);
        if (date && (!acc.nextAvailableDate || date < acc.nextAvailableDate)) acc.nextAvailableDate = date;
        acc.windows.push({ lotNumber: normalizeValue(lot.lotNumber), qtyKg: qty, window: getDispatchLotWindowLabel(lot) });
      } else if (isDispatchLotExpiredForDate(lot, dispatchDateIso)) {
        acc.expired += qty;
        acc.windows.push({ lotNumber: normalizeValue(lot.lotNumber), qtyKg: qty, window: getDispatchLotWindowLabel(lot), expired: true });
      }
      else acc.available += qty;
      acc.total += qty;
      return acc;
    }, { available: 0, expired: 0, future: 0, total: 0, nextAvailableDate: '', windows: [] });
    return {
      unit: 'kg',
      available: Number(totals.available.toFixed(2)),
      expired: Number(totals.expired.toFixed(2)),
      future: Number(totals.future.toFixed(2)),
      total: Number(totals.total.toFixed(2)),
      nextAvailableDate: totals.nextAvailableDate,
    ...
    await openIosSwal({
      ...
      didOpen: (popup) => {
        popup.addEventListener('click', (event) => {
          const btn = event.target.closest('[data-dispatch-xlsx-type]');
          if (!btn) return;
          const type = normalizeValue(btn.dataset.dispatchXlsxType);
          selected = type === 'cancel' ? '' : type;
          Swal.close();
        });
      }
    });
  ...
  const askDispatchXlsxTarget = async (targetType) => {
    const catalog = getDispatchXlsxTargetCatalog(targetType);
    let query = '';
    let picked = '';
    const selectedIngredients = {};
    const isIngredientMode = normalizeValue(targetType) === 'ingredient';
    const getSelectedIngredientsList = () => Object.values(selectedIngredients).map((item, idx) => ({ ...item, idx: idx + 1 }));
    const renderIngredientInputs = (popup) => {
      const host = popup.querySelector('#dispatchXlsxIngredientQtyList');
    ...
    const renderList = (popup) => {
      const listHost = popup.querySelector('#dispatchXlsxTargetList');
      if (!listHost) return;
    ...
    const result = await openIosSwal({
      ...
      didOpen: (popup) => {
        renderList(popup);
        popup.querySelector('#dispatchXlsxTargetSearch')?.addEventListener('input', (event) => {
          query = normalizeValue(event.target.value);
          renderList(popup);
        });
        popup.addEventListener('change', (event) => {
          ...
          if (!checkbox) return;
          const id = normalizeValue(checkbox.dataset.dispatchXlsxIngredientPick);
          if (!id) return;
          if (checkbox.checked) {
        ...
        popup.addEventListener('input', (event) => {
          const qtyInput = event.target.closest('[data-dispatch-xlsx-ingredient-qty]');
          if (!qtyInput) return;
          const id = normalizeValue(qtyInput.dataset.dispatchXlsxIngredientQty);
          if (!id || !selectedIngredients[id]) return;
          selectedIngredients[id].qty = normalizeValue(qtyInput.value);
        });
      },
      preConfirm: () => {
        if (isIngredientMode) {
          const selectedList = getSelectedIngredientsList()
            .map((item) => {
              ...
              return {
                ...item,
                qty: rawQty ? toDispatchXlsxPositiveQty(rawQty) : 0,
                useSourceQty: !rawQty
              };
            })
            .filter((item) => item.id);
          ...
          return {
            targetId: selectedList[0].id,
            ingredientTargets: selectedList,
            customKg: ''
          };
        }
        if (!picked) {
        ...
        const customKgRaw = normalizeValue(document.getElementById('dispatchXlsxMapQty')?.value);
        const customKg = customKgRaw ? toDispatchXlsxPositiveQty(customKgRaw) : 0;
        if (customKgRaw && customKg <= 0) {
          Swal.showValidationMessage('El multiplicador debe ser mayor a 0.');
          return false;
        }
        return {
          targetId: picked,
    ...
    await openIosSwal({
      ...
      didOpen: (popup) => {
        ...
        popup.addEventListener('click', async (event) => {
          ...
          if (!ask.isConfirmed) return;
          const cfg = getDispatchXlsxConfig();
          cfg.importHistory = getDispatchXlsxHistory().filter((row) => normalizeValue(row.id) !== id);
          await persistRepartoStore();
          render(popup);
        });
      }
  ...
  const recomputeDispatchXlsxDraftRows = (draft) => {
  ...
  const buildDispatchXlsxResolutionPreview = (draft) => {
    ...
    draft.rows.forEach((row) => {
      ...
      const effectiveDate = normalizeDispatchDateToken(row.invoiceDate) || toIsoDate();
      const mappedIngredients = Array.isArray(row.mappedIngredients) ? row.mappedIngredients : [];
      if (mappedIngredients.length) {
  ...
  const processDispatchXlsxRows = async () => {
    ...
    const activeRows = (Array.isArray(draft?.rows) ? draft.rows : []).filter((row) => !row.disabled);
    const invalidQtyRows = activeRows.filter((row) => !isDispatchXlsxRowQtyValid(row));
    if (invalidQtyRows.length) {
      await openIosSwal({ title: 'Cantidad inválida', html: '<p>Hay filas activas con cantidad 0 o inválida. Corregí la relación o deshabilitá esas filas antes de procesar.</p>', icon: 'warning' });
      return;
    }
    const missingTargetRows = activeRows.filter((row) => !normalizeValue(row.mappedTargetId));
    if (missingTargetRows.length) {
      await openIosSwal({ title: 'Filas sin relación', html: '<p>Hay filas activas sin producto relacionado. Relacionalas o deshabilitalas antes de procesar.</p>', icon: 'warning' });
      return;
    }
    const rows = activeRows.filter((row) => normalizeValue(row.mappedTargetId));
    if (!rows.length) {
    ...
    try {
      ...
      for (const groupRows of Object.values(byDispatch)) {
        ...
        for (const row of groupRows) {
          if (normalizeValue(row.mappedType) === 'ingredient' && Array.isArray(row.mappedIngredients) && row.mappedIngredients.length) {
            row.mappedIngredients.forEach((mappedItem) => {
              ...
              const expiredForRow = Number(rowPreview.expiredAvailable || 0);
              if (missingQty > 0.0001 && expiredForRow > 0.0001) {
                const chosen = normalizeValue(row.conflictResolutions?.[ingredientId]?.type);
                if (!chosen) {
                ...
                resolveDispatchXlsxIngredientExpiredConflict({
                  ingredientId,
                  requestedQty: Math.min(missingQty, expiredForRow),
          ...
          const expiredForRow = Number(rowProductPreview.expiredAvailable || 0);
          if (missingKg > 0.0001 && expiredForRow > 0.0001) {
            const chosen = normalizeValue(row.productConflictResolution);
            if (!chosen) {
              throw new Error(`conflict_pending_product:${row.id}:${recipeId}`);
            }
            await resolveDispatchXlsxRecipeExpiredConflict({
              recipeId,
              qtyKg: Math.min(missingKg, expiredForRow),
              dispatchDate,
              resolutionType: chosen === 'decommissioned' ? 'decommissioned' : 'retail_sale'
            });
          }
          const allocated = allocateDispatchLots(recipeId, qtyKg, dispatchDate);
          if (!allocated.hasStock && Array.isArray(allocated.futureLots) && allocated.futureLots.length) {
            throw new Error(`future_block_product:${row.id}:${recipeId}`);
          }
        ...
        state.reparto.registros[repartoId] = {
          ...
          managerProfiles: draft.managers.map((userId) => {
            const user = safeObject(state.users?.[userId]);
            return {
              id: userId,
              name: normalizeValue(user.fullName || user.email || userId),
              role: getDispatchUserRole(user)
            };
          }),
          comments,
          proofs: [],
  ...
  const renderDispatchXlsxCreate = (draft) => {
    ...
    const formatDispatchXlsxQtyWithUnit = (qty, unit) => {
      const amount = Number(qty || 0);
      const safeUnit = normalizeValue(unit || 'unidades');
      const singularMap = { kilos: 'kilo', kilo: 'kilo', unidades: 'unidad', unidad: 'unidad' };
      const pluralMap = { kilo: 'kilos', kilos: 'kilos', unidad: 'unidades', unidades: 'unidades' };
      const abs = Math.abs(amount);
      const isSingular = abs > 0.0001 && abs < 1.0001;
      if (singularMap[safeUnit] || pluralMap[safeUnit]) {
        const unitLabel = isSingular ? (singularMap[safeUnit] || safeUnit) : (pluralMap[safeUnit] || safeUnit);
    ...
    const body = xlsxPageSlice.length ? xlsxPageSlice.map((row) => {
      const mappedIngredients = Array.isArray(row.mappedIngredients) ? row.mappedIngredients : [];
      const rowQtyValid = isDispatchXlsxRowQtyValid(row);
      ...
      const rowHasStock = mappedIngredients.length
        ? rowQtyValid && !hasAnyIngredientPendingConflict
        : rowQtyValid && resolvedProductMissingQty <= 0.0001;
      ...
      const availableQty = Number(row.mappedAvailableKg || 0);
      const qtyClass = !rowQtyValid
        ? 'is-danger'
        : rowHasStock
        ? 'is-ok'
        : (!mappedIngredients.length && availableQty > 0.0001 ? 'is-warning' : 'is-danger');
      const expiredQty = Number(row.mappedExpiredQty || 0);
      const futureQty = Number(row.mappedFutureQty || 0);
      ...
      const qtyMap = row.mappedTargetTitle
        ? (mappedIngredients.length
          ? `<span class="dispatch-xlsx-qty-main">${multiplierLabel} → <span class="dispatch-xlsx-mapped-kg ${qtyClass}">${mappedIngredients.map((item) => {
            ...
            const hasExpiredConflict = Number(item.expired || 0) > 0.0001 && missing > 0.0001;
            const toneClass = missing <= 0.0001
              ? 'is-ok'
      ...
      const ingredientDetail = mappedIngredients.length
        ? `<div class="dispatch-xlsx-ingredient-breakdown">${mappedIngredients.map((item) => {
          const missingQty = getResolvedMissingQty(item);
          ...
          const resolutionBadge = resolutionType
            ...
            : '';
          const itemPreview = safeObject(resolutionPreview.ingredient?.[normalizeValue(row.id)]?.[normalizeValue(item.id)]);
        ...
        : '';
      let stockLine = row.mappedTargetTitle
        ? (() => {
          ...
          const missingParts = mappedIngredients.length
            ? mappedIngredients.map((item) => {
              ...
              return `${escapeHtml(formatDispatchXlsxQtyWithUnit(missingQty, item.unit || 'u'))} del producto ${escapeHtml(normalizeValue(item.title || item.id || ''))}`;
            }).filter(Boolean)
            : (() => {
              const missingQty = resolvedProductMissingQty;
              if (missingQty <= 0.0001) return [];
              return [`${escapeHtml(formatDispatchXlsxQtyWithUnit(missingQty, stockUnit))} del producto ${escapeHtml(capitalize(row.mappedTargetTitle || row.sourceProduct || ''))}`];
            })();
          if (!missingParts.length) return base;
          const canResolveProductConflict = !mappedIngredients.length && Number(row.mappedQty || 0) > Number(row.mappedAvailableKg || 0) && ((Number.isFinite(Number(rowProductPreview.expiredAvailable)) ? Number(rowProductPreview.expiredAvailable) : Number(expiredQty || 0)) > 0.0001 || Boolean(productConflictResolutionType));
      ...
      if (row.mappedTargetTitle && !rowQtyValid) stockLine = 'Cantidad inválida: debe ser mayor a 0.';
      ...
      nodes.dispatchView.querySelectorAll('[data-dispatch-xlsx-date]').forEach((input) => {
        window.flatpickr(input, {
          ...
          onChange: (_selected, dateStr) => {
            const rowId = normalizeValue(input.dataset.dispatchXlsxDate);
            const row = (Array.isArray(state.dispatchXlsxDraft?.rows) ? state.dispatchXlsxDraft.rows : []).find((item) => normalizeValue(item.id) === rowId);
            if (!row) return;
            row.invoiceDate = normalizeDispatchDateToken(dateStr) || toIsoDate();
            recomputeDispatchXlsxDraftRows(state.dispatchXlsxDraft);
  ...
  const renderDispatchCreate = (draft) => {
    ...
    const lineRows = draft.lines.map((line, idx) => {
      ...
      const alloc = allocateDispatchLots(line.recipeId, Number(line.qtyKg || 0), dispatchDateIso);
      line.allocations = alloc.allocations;
      const expiredLots = Array.isArray(alloc.expiredLots) ? alloc.expiredLots : getDispatchExpiredLots(alloc.allocations, dispatchDateIso);
      const futureLots = Array.isArray(alloc.futureLots) ? alloc.futureLots : [];
      const requestedKg = Number(line.qtyKg || 0);
      const availableKg = Number(alloc.availableKg || 0);
      const totalStockKg = Number(alloc.totalStockKg || 0);
      const missingKg = Number(alloc.missingKg || 0);
      const formatDispatchKg = (kg) => {
        const value = Number(kg || 0);
        return value > 0 && value < 0.01 ? value.toFixed(3) : value.toFixed(2);
      };
      const stockDateHint = [
        futureLots.length && alloc.nextAvailableDate
          ? `<small class="dispatch-date-hint">Hay ${formatDispatchKg(alloc.futureKg)} kg elaborados para fecha futura. Se puede repartir desde ${escapeHtml(formatIsoEs(alloc.nextAvailableDate))}.</small>`
          : '',
        expiredLots.length
          ? `<small class="dispatch-date-hint is-expired">Hay ${formatDispatchKg(alloc.expiredKg)} kg vencidos para ${escapeHtml(formatIsoEs(dispatchDateIso))}.</small>`
          : ''
      ].filter(Boolean).join('');
      let stockStatus = normalizeValue(line.recipeId)
        ? (requestedKg <= 0
      ...
      stockStatus = normalizeValue(line.recipeId)
        ? (requestedKg <= 0
      ...
      const expiryText = expiries.length === 1
        ...
        : (expiries.length
          ...
          : (blockedWindows.length ? blockedWindows.map((item) => `<span class="dispatch-expiry-text-danger">${escapeHtml(item)}</span>`).join('<br>') : 'Sin fecha'));
      ...
      const futureLot = alloc.hasStock ? null : futureLots[0];
      const futureHelpRow = futureLot
        ? `<tr class="dispatch-date-block-row"><td colspan="6"><p class="dispatch-expired-copy"><strong>Hay stock, pero no para esta fecha.</strong> El lote ${escapeHtml(futureLot.lotNumber || '-')} fue elaborado el ${escapeHtml(formatIsoEs(futureLot.productionDate || ''))}; podes repartirlo desde ${escapeHtml(formatIsoEs(futureLot.productionDate || ''))}${futureLot.expiryDate ? ` hasta ${escapeHtml(formatIsoEs(futureLot.expiryDate))}` : ''}. Cambia el dia de reparto con el calendario.</p></td></tr>`
        : '';
    ...
    if (window.flatpickr && dateInput) {
      window.flatpickr(dateInput, {
        ...
        disableMobile: true,
        onChange: (_selected, dateStr) => {
          const nextDate = normalizeValue(dateStr) || toIsoDate();
          if (nextDate === state.dispatchDraft?.dispatchDate) return;
          state.dispatchDraft.dispatchDate = nextDate;
  ...
  const deleteDispatchRecord = async (dispatchRow) => {
    ...
    try {
      ...
      await window.dbLaJamoneraRest.write(`${REPARTO_PATH}/registros/${dispatchId}`, null);
      const affectedRecipeIds = [...new Set((Array.isArray(dispatchRow?.products) ? dispatchRow.products : [])
        .map((product) => normalizeValue(product.recipeId))
        .filter(Boolean))];
  ...
  // === Agrupaciones de recetas =================================================
  // Las "recipeGroups" son carpetas con imagen para clasificar recetas (ej.
  // "Embutidos", "Salsas"). Cada receta puede tener `recipeGroupId` que apunta
  // a un grupo. La barra superior se renderiza con `renderRecipeGroups()` y
  // se filtra el listado por `state.activeRecipeGroupId`.
  // -----------------------------------------------------------------------------
  ...
  const uploadRecipeGroupImage = async (file) => {
    if (!file || !window.storageLaJamonera) return '';
    const ext = (file.name.split('.').pop() || 'jpg').toLowerCase();
    const refPath = `recetas/grupos/${Date.now()}_${Math.random().toString(36).slice(2, 8)}.${ext}`;
    const ref = window.storageLaJamonera.ref().child(refPath);
    await ref.put(file);
    return ref.getDownloadURL();
  };
  ...
  const renderRecipeGroups = () => {
    if (!nodes.recipeGroups) return;
    if (state.historyMode || state.dispatchMode) {
  ...
  const openRecipeGroupForm = async (existingId = '') => {
    ...
    // Usa el mismo image step que ingredientes/familias (Link / Subir / IA).
    const imageStep = window.laJamoneraIngredientesAPI?.imageStep;
    if (!imageStep) {
      // Fallback mínimo si la API no está cargada todavía.
  ...
  const deleteRecipeGroup = async (groupId) => {
    ...
    // Quitar groupId de las recetas asociadas.
  ...
  const renderEditor = async (recipeId, options = {}) => {
    ...
    const updateEditorPlan = async (formatInput = false) => {
      ...
      if (maxHelpNode) {
        const expiredExtra = !hasOnlyInfiniteStockForDate && Number(analysis.maxKgIncludingExpired || 0) > Number(analysis.maxKg || 0)
          ? ` <span class="produccion-expired-max-help">(con vencidos: ${Number(analysis.maxKgIncludingExpired || 0).toFixed(2)} kg)</span>`
          : '';
        const futureExtra = !state.sinTrazabilidad && analysis.nextAvailableDate
          ? ` <span class="produccion-expired-max-help">(hay stock desde ${formatIsoEs(analysis.nextAvailableDate)})</span>`
          : '';
      ...
      if (canConfirm) {
        ...
        qtyHelp.textContent = 'Modo visualización: podés revisar el desglose, pero no confirmar ni guardar borradores desde esta vista.';
      } else if (qty <= 0) {
        ...
        const futureHint = analysis.nextAvailableDate
          ? ` Hay stock que ingresa desde ${formatIsoEs(analysis.nextAvailableDate)}; cambiá la fecha o producí sin trazabilidad para el faltante.`
          : '';
        qtyHelp.textContent = `Para ${formatIsoEs(productionDate)} podés producir con trazabilidad hasta ${traceableMaxKg.toFixed(2)} kg.${futureHint}${conflictText ? ` ${conflictText}` : ''}`;
      }
    ...
    nodes.editor.addEventListener('click', async (event) => {
      ...
      if (event.target.closest('#produccionRecipeHistoryExpandAllRowsBtn')) {
        getRecipeHistoryRows().forEach((item) => {
          if (hasRegistroTracePreview(item)) state.historyTraceCollapse[item.id] = false;
        });
        renderRecipeHistory();
        return;
      }
      ...
      if (event.target.closest('#produccionRecipeHistoryExpandBtn')) {
        let rows = getRecipeHistoryRows();
        const collapseMap = { ...state.historyTraceCollapse };
        let expandedPage = 1;
        const EXPANDED_PAGE_SIZE = 12;
        const refreshExpandedRows = () => {
          rows = getRecipeHistoryRows();
          const liveIds = new Set(rows.map((item) => normalizeValue(item.id)));
          Object.keys(collapseMap).forEach((id) => {
            if (!liveIds.has(normalizeValue(id))) delete collapseMap[id];
          });
          rows.forEach((item) => {
            if (collapseMap[item.id] !== undefined) return;
            if (hasRegistroTracePreview(item)) collapseMap[item.id] = true;
          });
          expandedPage = Math.min(Math.max(1, expandedPage), Math.max(1, Math.ceil(rows.length / EXPANDED_PAGE_SIZE)));
        };
        ...
        const renderExpandedContent = (popup) => {
          ...
          if (!host) return;
          refreshExpandedRows();
          hydrateRegistroDetailsForRows(getPageRows(), () => renderExpandedContent(popup), EXPANDED_PAGE_SIZE);
          const traceableRows = rows.filter((item) => hasRegistroTracePreview(item));
          ...
          didOpen: (popup) => {
            ...
            popup.addEventListener('click', async (clickEvent) => {
              ...
              if (clickEvent.target.closest('#produccionRecipeExpandedHistoryExpandAllRowsBtn')) {
                rows.forEach((item) => {
                  if (hasRegistroTracePreview(item)) collapseMap[item.id] = false;
                });
                renderExpandedContent(popup);
                return;
              }
    ...
    const confirmProduction = async (opts = {}) => {
      ...
      try {
        ...
        recipeNombreComercial: normalizeValue(recipe.nombreComercial),
        productionDate: date,
  ...
  const refreshData = async ({ silent = false } = {}) => {
    ...
    await window.laJamoneraReady;
    failedRegistroDetailIds.clear();
    failedInventoryDetailIds.clear();
    const safeRead = async (path, fallback = {}) => {
      try {
        const value = await window.dbLaJamoneraRest.read(path);
        return value == null ? fallback : value;
    ...
    Object.values(safeObject(state.reparto.productIndex)).forEach((entry) => rebuildProductIndexEntryMetrics(entry));
  ...
  nodes.recipeGroups?.addEventListener('click', async (event) => {
    ...
    if (editBtn) {
      await openRecipeGroupForm(normalizeValue(editBtn.dataset.recipeGroupEdit));
      return;
    }
    const deleteBtn = event.target.closest('[data-recipe-group-delete]');
    if (deleteBtn) {
      await deleteRecipeGroup(normalizeValue(deleteBtn.dataset.recipeGroupDelete));
      return;
    }
    const manageBtn = event.target.closest('[data-recipe-group-manage]');
    if (manageBtn) {
      await openRecipeGroupAssign(normalizeValue(manageBtn.dataset.recipeGroupManage));
      return;
    }
  });
  ...
  nodes.list.addEventListener('click', async (event) => {
    ...
    const moreBtn = event.target.closest('[data-recipe-more]');
    if (moreBtn) {
      const allMenus = nodes.list?.querySelectorAll('.produccion-more-menu');
      const menu = moreBtn.closest('.produccion-more-wrap')?.querySelector('.produccion-more-menu');
      allMenus?.forEach((m) => { if (m !== menu) m.classList.add('d-none'); });
    ...
    if (sinTrazBtn) {
      const recipeId = sinTrazBtn.dataset.produceSinTrazabilidad;
      ...
      try {
        await renderEditor(recipeId, { mode: 'produce', sinTrazabilidad: true });
      } catch (error) {
  ...
  nodes.dispatchView?.addEventListener('click', async (event) => {
    ...
    if (event.target.closest('#produccionDispatchXlsxBtn')) {
      ...
      if (state.dispatchXlsxPendingResume && hasDispatchXlsxDraftChanges(state.dispatchXlsxDraft)) {
        ...
        if (!askResume.isConfirmed) {
          state.dispatchXlsxDraft = buildDispatchXlsxDraft();
        }
      } else if (!state.dispatchXlsxDraft || !hasDispatchXlsxDraftChanges(state.dispatchXlsxDraft)) {
        state.dispatchXlsxDraft = buildDispatchXlsxDraft();
      }
      state.dispatchXlsxPendingResume = false;
      renderDispatchXlsxCreate(state.dispatchXlsxDraft);
      return;
    }
    ...
    if (xlsxVehicleInput && state.dispatchXlsxDraft) {
      showDispatchXlsxVehicleSuggestions(xlsxVehicleInput);
      return;
    }
    if (event.target.closest('#dispatchXlsxAddVehicleBtn') && state.dispatchXlsxDraft) {
      const vehicle = await openCreateDispatchVehicle();
      if (vehicle) {
        state.dispatchXlsxDraft.vehicleId = vehicle.id;
        state.dispatchXlsxDraft.vehicleSearch = formatDispatchVehicleLabel(vehicle);
        renderDispatchXlsxCreate(state.dispatchXlsxDraft);
      }
      return;
    }
    if (event.target.closest('#dispatchXlsxManageVehiclesBtn') && state.dispatchXlsxDraft) {
    ...
    if (xlsxMapBtn && state.dispatchXlsxDraft) {
      ...
      const key = normalizeDispatchXlsxProductKey(row.sourceProduct);
      const cfg = getDispatchXlsxConfig();
      cfg.productRules[key] = {
        sourceProduct: row.sourceProduct,
        targetType: selectedType,
        targetId: pick.targetId,
        ingredientTargets: Array.isArray(pick.ingredientTargets) ? pick.ingredientTargets : [],
        useCustomKg: toDispatchXlsxPositiveQty(pick.customKg) > 0,
        customKg: toDispatchXlsxPositiveQty(pick.customKg)
      };
    ...
    if (expiredActionBtn) {
      ...
      const nextAlloc = allocateDispatchLots(recipeId, qtyRequested, dispatchDate);
      const stillExpired = (Array.isArray(nextAlloc.expiredLots) ? nextAlloc.expiredLots : getDispatchExpiredLots(nextAlloc.allocations, dispatchDate)).length > 0;
      if ((!nextAlloc.hasStock && !nextAlloc.futureLots?.length) || stillExpired) {
    ...
    if (event.target.closest('#dispatchSaveBtn')) {
      ...
      for (const line of draft.lines) {
        ...
        const recipe = safeObject(state.recetas[recipeId]);
        const allocated = allocateDispatchLots(recipeId, qtyKg, draft.dispatchDate);
        const expiredForDate = Array.isArray(allocated.expiredLots) ? allocated.expiredLots : getDispatchExpiredLots(allocated.allocations, draft.dispatchDate);
        ...
        if (allocated.futureLots?.length && !allocated.hasStock) {
          const nextDate = normalizeValue(allocated.nextAvailableDate);
          const windows = allocated.futureLots.slice(0, 4).map((lot) => `<li><strong>${escapeHtml(lot.lotNumber || '-')}</strong>: ${escapeHtml(getDispatchLotWindowLabel(lot))} - ${Number(lot.availableKg || 0).toFixed(2)} kg</li>`).join('');
          await openIosSwal({
            title: 'Stock fuera de fecha',
            html: `<p>${escapeHtml(capitalize(recipe.title || 'Receta'))} tiene stock, pero no se puede repartir el ${escapeHtml(formatIsoEs(draft.dispatchDate || ''))} porque todavia no estaba elaborado.</p>${nextDate ? `<p>Podes repartir desde <strong>${escapeHtml(formatIsoEs(nextDate))}</strong> o cambiar la fecha con el calendario.</p>` : ''}<ul class="text-start">${windows}</ul>`,
            icon: 'warning'
          });
          return;
        }
    ...
    if (event.target.closest('#produccionDispatchPrintBtn')) {
      ...
      const body = rows.flatMap((row, index) => {
        ...
        const groupedRows = groups.flatMap((group) => {
          const parentRow = `<tr class="dispatch-planilla-parent-row"><td colspan="6"><strong>${escapeHtml(group.label)}</strong></td></tr>`;
          const childRows = group.items.flatMap((item) => {
            const imageUrl = sanitizeImageUrl(item.recipeImageUrl || state.recetas?.[item.recipeId]?.imageUrl);
            const allocations = Array.isArray(item.allocations) && item.allocations.length
              ? item.allocations
        ...
        const detailRows = standalone.flatMap((item) => {
          const imageUrl = sanitizeImageUrl(item.recipeImageUrl || state.recetas?.[item.recipeId]?.imageUrl);
          const allocations = Array.isArray(item.allocations) && item.allocations.length
            ? item.allocations
            : [{ lotNumber: '-', qtyKg: item.qtyKg, expiryDate: '', productionId: '' }];
    ...
    if (event.target.closest('#produccionDispatchExcelBtn')) {
      ...
      const rows = getDispatchRows().flatMap((row) => {
        ...
        const locationRow = locationLabel
          ...
          : [];
        return [summary, ...groupedRows, ...detailRows, ...locationRow];
      });
      if (!rows.length) {
  ...
  nodes.dispatchView?.addEventListener('change', async (event) => {
    const xlsxInput = event.target.closest('#dispatchXlsxFileInput');
    if (xlsxInput && state.dispatchXlsxDraft) {
      ...
      try {
        ...
        renderDispatchXlsxCreate(state.dispatchXlsxDraft);
        uploadImageToStorage(file, 'reparto/xlsx').then(async (uploadedFileUrl) => {
    ...
    const xlsxToggle = event.target.closest('[data-dispatch-xlsx-row-enabled]');
    if (xlsxToggle && state.dispatchXlsxDraft) {
      const rowId = normalizeValue(xlsxToggle.dataset.dispatchXlsxRowEnabled);
      const row = (Array.isArray(state.dispatchXlsxDraft.rows) ? state.dispatchXlsxDraft.rows : []).find((item) => item.id === rowId);
      if (!row) return;
      row.disabled = !xlsxToggle.checked;
      const key = normalizeDispatchXlsxProductKey(row.sourceProduct);
      const cfg = getDispatchXlsxConfig();
      if (row.disabled) {
        const ask = await openIosSwal({
        ...
        if (ask.isDenied) cfg.disabledProducts[key] = true;
      } else {
        delete cfg.disabledProducts[key];
      }
      await persistRepartoStore();
    ...
    const xlsxDateInput = event.target.closest('[data-dispatch-xlsx-date]');
    if (xlsxDateInput && state.dispatchXlsxDraft) {
      const rowId = normalizeValue(xlsxDateInput.dataset.dispatchXlsxDate);
      const row = (Array.isArray(state.dispatchXlsxDraft.rows) ? state.dispatchXlsxDraft.rows : []).find((item) => normalizeValue(item.id) === rowId);
      if (!row) return;
      row.invoiceDate = normalizeDispatchDateToken(xlsxDateInput.value) || toIsoDate();
      recomputeDispatchXlsxDraftRows(state.dispatchXlsxDraft);
      renderDispatchXlsxCreate(state.dispatchXlsxDraft);
      return;
    }
  ...
  const showDispatchXlsxVehicleSuggestions = (vehicleInput) => {
    if (!state.dispatchXlsxDraft || !vehicleInput) return;
    const query = normalizeLower(vehicleInput.value);
    state.dispatchXlsxDraft.vehicleSearch = normalizeValue(vehicleInput.value);
    state.dispatchXlsxDraft.vehicleId = '';
    const hidden = nodes.dispatchView?.querySelector('#dispatchXlsxVehicleSelect');
    if (hidden) hidden.value = '';
    const list = Object.values(safeObject(state.reparto.vehicles || {}))
      .filter((item) => item.enabled !== false)
      .filter((item) => {
    ...
    suggest.onclick = async (ev) => {
      ...
      if (pick) {
        const vehicle = getDispatchVehicle(pick.dataset.dispatchXlsxVehiclePick);
        if (!vehicle.id) return;
        state.dispatchXlsxDraft.vehicleId = vehicle.id;
        state.dispatchXlsxDraft.vehicleSearch = formatDispatchVehicleLabel(vehicle);
        vehicleInput.value = state.dispatchXlsxDraft.vehicleSearch;
        if (hidden) hidden.value = vehicle.id;
        closeDispatchSuggests();
        return;
      }
      if (ev.target.closest('[data-dispatch-xlsx-vehicle-create]')) {
        closeDispatchSuggests();
        const created = await openCreateDispatchVehicle();
        if (created) {
          state.dispatchXlsxDraft.vehicleId = created.id;
          state.dispatchXlsxDraft.vehicleSearch = formatDispatchVehicleLabel(created);
          renderDispatchXlsxCreate(state.dispatchXlsxDraft);
        }
      }
    };
  ...
  nodes.dispatchView?.addEventListener('input', async (event) => {
    if (!state.dispatchCreateMode && !state.dispatchXlsxMode) return;
    const activeIsXlsx = state.dispatchXlsxMode;
    const activeDraft = activeIsXlsx ? state.dispatchXlsxDraft : state.dispatchDraft;
    if (!activeDraft) return;
    const clientInput = event.target.closest('#dispatchClientInput');
    if (!activeIsXlsx && clientInput) {
    ...
    if (activeIsXlsx) {
      const xlsxManagersSearchInput = event.target.closest('#dispatchXlsxManagersSearch');
      if (xlsxManagersSearchInput) {
        activeDraft.managerSearch = normalizeValue(xlsxManagersSearchInput.value);
        const query = normalizeLower(xlsxManagersSearchInput.value);
    ...
    const vehicleInput = event.target.closest('#dispatchVehicleInput');
    if (!activeIsXlsx && vehicleInput) {
      showDispatchVehicleSuggestions(vehicleInput);
      return;
    }

    const productInput = event.target.closest('[data-dispatch-product-search]');
    if (activeIsXlsx) return;
    if (!productInput) return;
    showDispatchProductSuggestions(productInput);
  });

  nodes.dispatchView?.addEventListener('focusin', (event) => {
    if (!state.dispatchXlsxMode || !state.dispatchXlsxDraft) return;
    const xlsxVehicleInput = event.target.closest('#dispatchXlsxVehicleInput');
    if (!xlsxVehicleInput) return;
    showDispatchXlsxVehicleSuggestions(xlsxVehicleInput);
  });
  ...
  document.addEventListener('click', (event) => {
    ...
    if (event.target.closest('#dispatchXlsxVehicleInput')) return;
  ...
  produccionModal.addEventListener('shown.bs.modal', async () => {
    ...
    try {
      ...
      if (window.flatpickr && nodes.historyRange) {
        nodes.historyRange._flatpickr?.destroy?.();
        const locale = window.flatpickr.l10ns?.es || undefined;
